...
from IEFI import IEFI
from HR import HR
from SCGR import SCGR
...
class VRWKV_SpatialMix(nn.Module):
    ...
    def get_rwkv_indices(self, h, w, start='top-left', direction='horizontal'):
        indices = []
        if start == 'top-left':
            row_start = 0
            col_start = 0
    ...
    def jit_func(self, x, resolution, scan_scheme):
        ...
        rwkv_order = self.get_rwkv_indices(h, w, start=start, direction=direction)
    ...
    def forward(self, x, resolution):
        ...
        for j in range(self.recurrence):
            if j % 2 == 0:
                ...
                rwkv_order = self.get_rwkv_indices(new_h, new_w, start=selected_scheme[0],
                                                       direction=selected_scheme[1])
                k = rearrange(k, 'b (h w) c -> b c h w', h=h, w=w)
                k = rearrange(k, 'b c h w -> b c (h w)')[..., rwkv_order]
                k = rearrange(k, 'b c (h w) -> b (h w) c', h=new_h, w=new_w)

                v = rearrange(v, 'b (h w) c -> b c h w', h=h, w=w)
                v = rearrange(v, 'b c h w -> b c (h w)')[..., rwkv_order]
...
class PyramidRWKV_enc(nn.Module):
    def __init__(self, img_size=512, outer_dims=None, in_chans=1, mlp_ratio=4., qkv_bias=False,
        ...
        self.resnet50 = get_resnet50_model()
        self.iefi_fuse = nn.ModuleList([IEFI(outer_dims[i]) for i in range(4)])
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            mid_out = self.iefi_fuse[i](mid_out, x1[i])
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_bn_relu = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            CLAI(out_channels, out_channels),
...
class TSFANet(nn.Module):
    def __init__(self, channels, num_classes=None, img_size=None, in_chans=3):
        super(TSFANet, self).__init__()

        self.RWKV_backbone = PyramidRWKV_enc(img_size=img_size, outer_dims=channels, in_chans=in_chans)
        ...
        self.conv = nn.Conv2d(channels[0], num_classes, kernel_size=1, bias=False)
        self.hr = HR(in_ch=channels[0],out_ch=channels[0])
        self.scgr = SCGR(c=channels[3])
    def forward(self, x):
        _, _, hei, wid = x.shape
        outputs = self.RWKV_backbone(x)
        t1, t2, t3, t4 = outputs[0], outputs[1], outputs[2], self.scgr(outputs[3])

        d4 = self.decode4(t4, t3)
        d3 = self.decode3(d4, t2)
        d2 = self.decode2(d3, t1)
        d2_up = self.up(d2)
        out = self.conv(self.hr(d2_up))
