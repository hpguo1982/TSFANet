...
def get_model(args2):
    model = TSFANet(channels=[64, 128, 256, 512], num_classes=args2.nclass, img_size=args2.crop_size[0], in_chans=3)
...
def train():
    ...
    # ==== 加载预训练权重 ====
    if args2.pretrained and os.path.exists(args2.pretrained):
        print(f"🔍 Loading pretrained weights from: {args2.pretrained}")
        try:
            state_dict = torch.load(args2.pretrained, map_location='cuda')
            model.load_state_dict(state_dict)
            print("✅ Successfully loaded pretrained weights.")
        except RuntimeError as e:
            print("⚠️ RuntimeError while loading pretrained weights:", e)
            print("🔁 Trying to load with strict=False ...")
            model.load_state_dict(state_dict, strict=False)
            print("✅ Loaded with strict=False.")

    data_train = Data(train=True, dataset=args2.dataset, crop_szie=args2.crop_size, nclass=args2.nclass)
    dataloader_train = DataLoader(
        ...
        drop_last=False,
        sampler=None)
    data_val = Data(train=False, dataset=args2.dataset, crop_szie=args2.crop_size, nclass=args2.nclass)
    ...
    for epoch in range(args2.end_epoch):
        ...
        model.train()
        setproctitle.setproctitle("TSFANet:" + str(epoch) + "/" + "{}".format(args2.end_epoch))
        ...
        if epoch % 2 == 0:
            ...
            F1 = Eval(dataloader_val, model, args2)
            if F1 > maxF1:
                maxF1 = F1
                print("best")
                torch.save(model.state_dict(), '/home/weimengru/dataset/CrackMap/data/929weight.pkl')
